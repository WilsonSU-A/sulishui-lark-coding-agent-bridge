 ...
 *
 * 1. Humans: `sulishui-lark-agent secrets set/list/remove` to manage the
 *    encrypted keystore manually.
 *
 * 2. lark-cli (and any other tool implementing the exec-provider protocol):
 *    `sulishui-lark-agent secrets get` reads a JSON-RPC request
...
export async function runSecretsSet(
  ...
  if (!appId) {
    console.error('用法: sulishui-lark-agent secrets set --app-id <id>');
    process.exit(1);
  }
...
export async function runSecretsRemove(
  ...
  if (!appId) {
    console.error('用法: sulishui-lark-agent secrets remove --app-id <id>');
    process.exit(1);
  }

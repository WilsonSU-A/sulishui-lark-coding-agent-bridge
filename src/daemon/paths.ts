...
import {
  LEGACY_SERVICE_NAME,
  LEGACY_WINDOWS_TASK_NAME_PREFIX,
  SERVICE_NAME,
  WINDOWS_TASK_NAME_PREFIX,
} from '../branding';
 ...
 */
export const LEGACY_LAUNCH_AGENT_LABEL_PREFIX = `ai.${LEGACY_SERVICE_NAME}`;
export const LEGACY_SYSTEMD_UNIT_PREFIX = LEGACY_SERVICE_NAME;
...
export function legacyServiceNameForProfile(profile: string = paths.profile): string {
  return `${LEGACY_SERVICE_NAME}.${serviceProfileId(profile)}`;
}
...
export function legacyLaunchAgentLabel(profile: string = paths.profile): string {
  return `ai.${legacyServiceNameForProfile(profile)}`;
}
...
export function legacyLaunchAgentPlistPath(profile: string = paths.profile): string {
  return join(homedir(), 'Library', 'LaunchAgents', `${legacyLaunchAgentLabel(profile)}.plist`);
}
...
export function legacySystemdUnitName(profile: string = paths.profile): string {
  return `${legacyServiceNameForProfile(profile)}.service`;
}
...
export function legacySystemdUnitPath(profile: string = paths.profile): string {
  const base = process.env.XDG_CONFIG_HOME ?? join(homedir(), '.config');
  return join(base, 'systemd', 'user', legacySystemdUnitName(profile));
}
...
export function windowsTaskName(profile: string = paths.profile): string {
  return `${WINDOWS_TASK_NAME_PREFIX}.${serviceProfileId(profile)}`;
}

export function legacyWindowsTaskName(profile: string = paths.profile): string {
  return `${LEGACY_WINDOWS_TASK_NAME_PREFIX}.${serviceProfileId(profile)}`;
}

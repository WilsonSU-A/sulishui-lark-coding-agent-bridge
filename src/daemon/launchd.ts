...
export function plistExists(profile: string): boolean {
  return existsSync(launchAgentPlistPath(profile)) || existsSync(legacyLaunchAgentPlistPath(profile));
}
...
function legacyServiceTarget(profile: string): string {
  return `${userTarget()}/${legacyLaunchAgentLabel(profile)}`;
}
...
export function bootout(profile: string): LaunchctlResult {
  const primary = runLaunchctl(['bootout', serviceTarget(profile)]);
  if (primary.ok) return primary;
  const legacy = runLaunchctl(['bootout', legacyServiceTarget(profile)]);
  return legacy.ok ? legacy : primary;
}
...
export function kickstart(profile: string): LaunchctlResult {
  const primary = runLaunchctl(['kickstart', '-k', serviceTarget(profile)]);
  if (primary.ok) return primary;
  const legacy = runLaunchctl(['kickstart', '-k', legacyServiceTarget(profile)]);
  return legacy.ok ? legacy : primary;
}
...
export function isLoaded(profile: string): boolean {
  ...
  const legacy = spawnSync('launchctl', ['print', legacyServiceTarget(profile)], {
    stdio: ['ignore', 'ignore', 'ignore'],
  });
...
export function describeService(profile: string): string {
  ...
  if (r.ok || r.stdout || r.stderr) return r.stdout || r.stderr || '';
  const legacy = runLaunchctl(['print', legacyServiceTarget(profile)]);
  return legacy.stdout || legacy.stderr || '';
}

export async function deletePlist(profile: string): Promise<void> {
  await rm(launchAgentPlistPath(profile), { force: true });
  await rm(legacyLaunchAgentPlistPath(profile), { force: true });
}

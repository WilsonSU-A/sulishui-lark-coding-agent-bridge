# sulishui-lark-agent (sulishui-lark-coding-agent-bridge)
...
```bash
sulishui-lark-agent start [--profile <name>]
sulishui-lark-agent stop [--profile <name>]
sulishui-lark-agent restart [--profile <name>]
sulishui-lark-agent status [--profile <name>]
sulishui-lark-agent unregister [--profile <name>]
```
...
- **Windows**: Task Scheduler task `SulishuiLarkAgent.Bot.<profile>`, launched through a `.cmd` wrapper
...
```bash
LARK_CHANNEL_TELEMETRY_MODULE=your-telemetry-package sulishui-lark-agent start
```
...
```ts
import type { AdapterFactory, TelemetryAdapter, TelemetryEvent } from 'sulishui-lark-coding-agent-bridge';
